...
export default async function InventoryDashboard() {
  ...
  const availableBikes = bikes.filter((bike: any) => !bike.sold);
  const newCount = availableBikes.filter((bike: any) => bike.category === "new").length;
        ...
        <div className="bg-gradient-to-r from-orange-500 to-orange-600 rounded-2xl shadow-xl p-8 text-white">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-lg opacity-90">จัดการและดูรายการสินค้าในระบบ</p>
            </div>
            <div className="flex gap-3">
              <Button asChild className="bg-white text-orange-600 hover:bg-gray-100 font-bold shadow-lg">
                <Link href="/inventory/add">
                  <Plus className="mr-2 h-5 w-5" />
                  เพิ่มสินค้า
                </Link>
              </Button>
              <Button asChild variant="outline" className="bg-white/20 text-white border-white/30 hover:bg-white/30 backdrop-blur-sm font-bold">
...
function DashboardCard({
  href, title, description, count, icon: Icon, gradient, hoverGradient,
}: {
  href: string; title: string; description: string; count: number;
  icon: any; gradient: string; hoverGradient: string;
}) {
  return (
