...
const BirthdayNotification = () => {
  ...
  const loadBirthdays = useCallback(async () => {
    ...
    try {
      ...
      console.log("📋 Customers with DOB:", customers.filter((c: ICustomer) => c.dob).length);
      
      const upcoming = findUpcomingBirthdays(customers);
    ...
    setLoading(false);
  }, []); // ✅ เพิ่ม dependency array ว่าง

  useEffect(() => {
    loadBirthdays();
    const interval = setInterval(loadBirthdays, 5 * 60 * 1000);
    return () => clearInterval(interval);
  }, [loadBirthdays]); // ✅ depend on loadBirthdays
  ...
  const findUpcomingBirthdays = (customers: ICustomer[]): IBirthdayCustomer[] => {
    ...
    return customers
      .filter(c => c.dob) // มีวันเกิด
      .map(c => {
        const dob = new Date(c.dob!);
        const birthMonth = dob.getMonth() + 1;
        const birthDay = dob.getDate();
        
        console.log(`👤 ${c.name}: DOB = ${birthDay}/${birthMonth}, Today = ${todayDay}/${todayMonth}`);

        let daysUntil = 0;

        if (birthMonth === todayMonth && birthDay === todayDay) {
          daysUntil = 0; // วันนี้!
          console.log(`🎉 ${c.name} - TODAY!`);
        } else {
          // สร้างวันเกิดในปีนี้
          ...
          console.log(`📆 ${c.name} - ${daysUntil} days`);
        }

        return { ...c, daysUntil };
      })
      .filter(c => c.daysUntil >= 0 && c.daysUntil <= 7) // แสดงเฉพาะ 0-7 วัน (7 วันก่อนถึงวันเกิด)
      .sort((a, b) => a.daysUntil - b.daysUntil); // เรียงตามความใกล้
  };
            ...
            {birthdays.length > 0 && (
              <div className="p-3 border-t bg-gray-50">
                <div className="text-center text-sm text-gray-600">
                  {todayCount > 0 && (
                    <span className="text-pink-600 font-medium">วันนี้ {todayCount} คน</span>
                  )}
